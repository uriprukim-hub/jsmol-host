...
this.showSpinSymmetry = "spin".equals(this.fillRange);
...
} else if (this.key.startsWith("_space_group_transform") || this.key.startsWith("_parent_space_group") || this.key.startsWith("_space_group_magn_transform") || this.key.contains("_spin_transform_")) {
...
if (this.key.contains("_spin_transform_spinframe_pp_abc")) {
this.addCellType("spin", this.field, false);
...
}this.appendUnitCellInfo("spinFrame=" + this.spinFrame);
...
function(){
if (this.magCenterings != null || this.lstSpinFrames != null) this.addLatticeVectors();
...
function(){
if (this.lstSpinFrames != null) {
if (this.asc.getSymmetry().addSpinLattice(this.lstSpinFrames)) this.appendLoadNote("Note! spin lattice added: " + this.lstSpinFrames);
this.lstSpinFrames = null;
return;
}this.lattvecs = null;
...
var f = -1;
var f0 = -1;
if ((f0 = f = this.fieldProperty(this.key2col[1])) != -1 || (f = this.fieldProperty(this.key2col[49])) != -1 || (f = this.fieldProperty(this.key2col[73])) != -1 || (f0 = f = this.fieldProperty(this.key2col[20])) != -1 || (f = this.fieldProperty(this.key2col[21])) != -1 || (f0 = this.fieldProperty(this.key2col[63])) != -1) {
...
ouvw = field;
continue;
}ouvw = this.asc.getXSymmetry().getBaseSymmetry().transformUVW(field, this.spinFrame);
field = oxyz + "(" + ouvw + ")";
timeRev = 0;
}
...
if (this.lstSpinFrames == null) {
this.lstSpinFrames =  new JU.Lst();
}this.lstSpinFrames.addLast(sxyz + "(" + suvw + ")");
suvw = null;
}sxyz = null;
}}
...
;//5.0.1-v5 Fri Feb 28 18:35:23 CST 2025
